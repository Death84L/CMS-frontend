...
const getIcon = (dataType) => {
  switch (dataType) {
    case "number":
      return <FaHashtag className="text-orange-500" />;
    case "text":
    ...
    >
      {data.map((schema) => (
        <div key={schema._id} className="mb-6">
          <div className="bg-gradient-to-r from-orange-50 to-orange-100 rounded-xl shadow-lg p-6 transition transform hover:scale-105 duration-300 ease-in-out hover:shadow-2xl">
            <h2 className="text-2xl font-bold mb-4 text-orange-900 border-b-2 border-orange-300 pb-2">
              {schema.tableName}
            </h2>
            <ul className="space-y-4">
              {schema.fields.map((field, index) => (
                <li
                  key={index}
                  className="flex justify-between items-center text-orange-900 bg-orange-200 p-3 rounded-lg shadow-inner"
                >
